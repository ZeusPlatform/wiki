...
```js
{
  ...
  plugins: [
    ...
    /* config.plugin('html-inline-source') */
...
let config = {
  ...
  chainWebpack: config => {
    config
      .entry('inline-source')
      .add('./src/inline.js') // 增加一个入口目的，这部分代码希望内联到页面中
    ...
    if (process.env.NODE_ENV === 'production') {
      config
        .optimization
        .minimizer([new TerserPlugin({ terserOptions: { compress: { drop_console: false } } })])
        .splitChunks({
          cacheGroups: {
            vendors: {
              test: /[\\/]node_modules[\\/]/,
        ...
        .runtimeChunk({ name: 'runtime' }) // 抽出运行时，避免多入口时模块重复加载
    }

    config
      .plugin('html')
      .tap(args => {
        args[0].inlineSource = '(inline-source|common).+\\.js$' // 需要内联的js文件
        return args
      })
      .end()
      ...
      .after('html')
      .end()
      .plugin('preload') // 去除内联代码的link加载
      .tap(args => {
        args[0].fileBlacklist = args[0].fileBlacklist || []
        args[0].fileBlacklist.push(/(inline-source|common).+\.js$/)
        return args
      })
  }
}
...
## [#](https://cli.vuejs.org/zh/guide/webpack.html#链式操作-高级)链式操作 (高级)
...
### [#](https://cli.vuejs.org/zh/guide/webpack.html#修改-loader-选项)修改 Loader 选项
...
### [#](https://cli.vuejs.org/zh/guide/webpack.html#添加一个新的-loader)添加一个新的 Loader
...
## [#](https://cli.vuejs.org/zh/guide/webpack.html#审查项目的-webpack-配置)审查项目的 webpack 配置
...
## [#](https://cli.vuejs.org/zh/guide/webpack.html#以一个文件的方式使用解析好的配置)以一个文件的方式使用解析好的配置
